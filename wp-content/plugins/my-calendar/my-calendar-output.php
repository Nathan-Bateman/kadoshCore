...
function my_calendar_next_link( $cur_year, $cur_month, $cur_day, $format, $time = 'month', $num_months = 1 ) {
	...
	$next_events = ( get_option( 'mc_next_events' ) == '' ) ? "<span class='maybe-hide'>" . __( "<i class='fa fa-chevron-circle-right' aria-hidden='true'></i>", 'my-calendar' ) . "</span>" : stripcslashes( get_option( 'mc_next_events' ) );
...
function my_calendar_prev_link( $cur_year, $cur_month, $cur_day, $format, $time = 'month', $num_months = 1 ) {
	...
	$previous_events = ( get_option( 'mc_previous_events' ) == '' ) ? "<span class='maybe-hide'>" . __( "<i class='fa fa-chevron-circle-left' aria-hidden='true'></i>", 'my-calendar' ) . "</span>" : stripcslashes( get_option( 'mc_previous_events' ) );
	if ( $num_months <= 1 || $format != "list" ) {
		if ( $cur_month == 1 ) {
			$pMonth = 12;
			$pYr    = $last_year;
